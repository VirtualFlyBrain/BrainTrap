...
function checkEnterTree(event) {
    var keycode;
    if (event && event.which) keycode = event.which;
    ...
    {
        selectAll();
        document.tree_search_form.submit();
        return false;
    }
    else return true;
}

function checkEnterQuick(event) {
    var keycode;
    if (event && event.which) keycode = event.which;
    else if (window.event) keycode = window.event.keycode;
    else return true;
    ...
    if (keycode == 13)
    {
        document.quick_search_form.submit();
        return false;
    }
    else return true;
}
...
function toggletree(treeid)
{
    ...
    if (elem.style.display == 'none')
    {
        ...
        ielem.src = image_dir + 'minus.png';
    }
    else
    {
        elem.style.display = 'none';
        var ielem = document.getElementById(treeid + '_i');
        ielem.src = image_dir + 'plus.png';
    }
