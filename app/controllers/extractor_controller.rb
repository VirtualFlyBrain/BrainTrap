require 'extractor_thread'

class ExtractorController < ApplicationController
  def index

    @stacks = Stack.find(:all, :order => 'id')

    for stack in @stacks
      stack[:file_src] = "#{EXTRACTOR_SRC_FOLDER}#{stack[:name]}.lsm"
      stack[:file_dest] = "#{EXTRACTOR_DEST_FOLDER}#{stack[:id]}_#{stack[:name]}/"
      stack[:exist_src] = File.exist?(stack[:file_src])
      stack[:exist_dest] = File.exist?(stack[:file_dest])

      if stack[:id] == $EXTRACTOR_CURRENT
        stack[:status] = "processing: #{$EXTRACTOR_PROGRESS} #{$EXTRACTOR_POINT}"
      elsif stack[:exist_src]
        if stack[:exist_dest]
          stack[:status] = 'complete'
        else
          stack[:status] = 'waiting'
        end
      else
        stack[:status] = 'error'
      end
        ...
        Thread.new {
          begin
            ...
            for s in $EXTRACTOR_STACKS
              $EXTRACTOR_POINT = '1';
              if s[:status] == 'waiting' && !File.exist?(stack[:file_dest])
                ...
                Stack.update(s[:id], :num_images => zn, :max_res_x => width, :max_res_y => height, :num_channels => channels)
